...
- [x] **AUTH-01**: A user signs in with email only — a magic link is sent via Resend and clicking it establishes a session.
- [x] **AUTH-02**: The magic link is single-use and time-limited; a second or expired click shows a clear message with a resend path.
- [x] **AUTH-03**: A signed-in user persists across browser sessions and devices via a secure session cookie.
- [x] **AUTH-04**: Each user has exactly one brain; signing in always routes them to their own brain, auto-provisioned on first sign-in.
- [x] **AUTH-05**: A user's brain data cannot be read or queried by another user — isolation is enforced by session-derived source-scoping: every gbrain query is hard-scoped to the authenticated user's gbrain `source_id`. *(Amended 2026-05-22: the original "gbrain row-level security" mechanism was disproven by Phase 6 research — gbrain RLS only denies the anon key; see `06-RESEARCH.md`.)*
- [x] **AUTH-06**: Routes that expose tenant data require an authenticated session; unauthenticated requests are redirected to sign-in.
- [x] **AUTH-07**: A user can sign out, ending the session.
- [x] **AUTH-08**: The user store (email → brain mapping, session and magic-link records) lives in Supabase Postgres, not a local file.
- [x] **AUTH-09**: Magic-link requests are rate-limited per email address to prevent abuse and email flooding.
...
| AUTH-01 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-02 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-03 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-04 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-05 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-06 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-07 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-08 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
| AUTH-09 | Phase 6: Auth + Multi-Tenant Isolation | Complete |
