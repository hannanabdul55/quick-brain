...
| 010 | per-tenant-engine-rls | standard | Given gbrain's auto-enabled RLS (spike 005), when a single long-lived `createEngine` instance serves multiple tenants via `SET LOCAL app.current_tenant = ...` per query, then does retrieval correctly isolate tenants — or must we call `createEngine` per request for correctness? Architectural pivot for Phase 4 + Phase 7. | **PENDING** | rls, multi-tenant, in-process, architecture, isolation |
