...
| 009 | vercel-fluid-coldstart | standard | Given gbrain bundled as a `package.json` dep loaded under Bun, when measuring (Bun init + gbrain import + Supabase connect + first hybridSearch), then what is the realistic cold-start latency contributors (warm baseline from spike 006 = 1.34s end-to-end) — extrapolated to Vercel Fluid Compute instance behavior? | **PENDING** | vercel, fluid-compute, bun, cold-start, phase-4-precondition |
