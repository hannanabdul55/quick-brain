...
- **`lib/gbrain/tenant-scoped.ts` wrapper layer + ESLint rule are LANDED in production (2026-06-01, commits 9ec60f3 / 767858e / c6abef5 / 26ca184).** Phase 7 plans can build on top of it.
- **The shipped API shape diverges from spike-011's blueprint.** Production splits each operation into TWO variants: session-derived (`tenantSafeHybridSearch(query, opts)` — no tenantId arg, calls `resolveTenant()` internally; used by app routes) and explicit-sourceId (`tenantSafeHybridSearchExplicit(sourceId, query, opts)` — used by Inngest jobs / provisioning / scripts). The spike's `tenantSafeX(tenantId, ...)` shape was rejected during landing because production's `resolveTenant()` takes no args and D-11 forbids accepting a tenant id from caller input.
- **Tenant→sourceId resolution goes through `lib/auth/resolve-tenant.ts::resolveTenant()`.** Function takes NO args; reads `qb_session` cookie. Never trust `params.id` directly. Phase 6 chokepoint.
...
| **v2.0 multi-tenant isolation** | **[references/v20-multi-tenant-isolation.md](references/v20-multi-tenant-isolation.md)** | RLS does NOT protect tenants (BYPASSRLS role). All isolation via app-layer per-call sourceId. Single shared engine survives 20 interleaved concurrent queries with 0 leaks. **Wrapper + lint LANDED 2026-06-01** with a session-derived/explicit-sourceId split (deviation from spike-011's tenantId-as-arg blueprint — D-11 forced the change). One-brain-many-sources is a deliberate deviation from gbrain's documented data-owner=brain rule; gbrain has no native multi-tenant primitive. |
